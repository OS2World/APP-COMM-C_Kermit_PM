...
extern int flow, local, mdmtyp, quiet, speed, parity, seslog, ttyfd, network;
...
/*
 * The following four lines added at request of Peter Mauzey, but I'm not
 * sure this is where they're really supposed to go.
 */
    if (augmdmtyp == n_ATTMODEM || augmdmtyp == n_ATTDTDM ) {
...
#ifdef ATT6300
              if (debug) {
            if (didWeGet(lbuf,"\177\177")) status = CONNECTED;
                      }
#endif
              break;
          case n_CERMETEK:
