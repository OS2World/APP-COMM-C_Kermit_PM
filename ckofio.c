...
/*
26-Jan-90 C.P.Armstrong zsout, zsoutl and zsoutx modified to filter writes to 
                        stdout.  This enables the strings to be written to the
                        PM screen.
*/
...
void zkself() {              /* For "bye", but no guarantee! */
...
zchout(n,c) int n; char c; {
    ...
    if (n == ZSFILE)
        ...
        if(fp[n]==stdout)
            ...
            return(write(fileno(fp[n]),&c,1)); /* Use unbuffered for */
        }                                      /* session log */
    else 
...
void zdelet(name) char *name; {
    unlink(name);
}
 
/*  Z R T O L  --  Convert remote filename into local form  */
void zrtol(name,name2) char *name, *name2; {
char *s,*r;
int i;
    i = 0;
    s = name;
